            ...
            <div className="mx-auto max-w-7xl px-6 py-6 md:px-12 md:py-8">
              ...
              <label className="relative block border-b border-foreground">
                ...
                <input
                  ...
                  placeholder="Search by country, fit or color"
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {["Argentina", "Brazil", "France", "Japan", "Archive"].map((term) => (
                  <button
                    key={term}
                    type="button"
                ...
                {searchResults.length > 0 ? (
                  ...
                  <div className="border-y border-border py-12">
                    ...
                    <p className="mt-4 max-w-md text-sm leading-relaxed text-muted-foreground">
                      Try a country, season, color or fit. Archive requests can
                      also be sent through the contact page.
                    </p>
                  </div>
                )}
              </div>
              ...
              {items.length === 0 ? (
                ...
                <div className="divide-y divide-border border-y border-border">
                  {items.map(({ product, size, quantity }) => (
                    <article key={`${product.id}-${size}`} className="grid grid-cols-[86px_1fr] gap-4 py-5">
                      <Link
                        to="/products/$productId"
                        params={{ productId: product.id }}
                        onClick={() => setIsCartOpen(false)}
